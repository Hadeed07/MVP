...
class SpinePipeline:
    ...
    def get_recommendations(self, isbn13):
        """
        Given an isbn13 already in the Chroma index, retrieves its stored
        embedding and queries for the most similar books (excluding itself).
        Returns an empty list if the isbn13 isn't in the index.
        """
        ...
        if not stored["ids"]:
            return []  # isbn13 not found in the Chroma index
    ...
    def results(self, image):
        ...
        for match in matches:
            match["recommendations"] = self.get_recommendations(match["isbn13"])
